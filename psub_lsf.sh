...
psub_submit() {
    ...
    n=$(expr "$NNODES" \* "$PPN")
    bsub -q "$QUEUE" -l NMIWATCHDOG_OFF=1 -n $n -R "{$select $span}" -W 00:$TIME_LIMIT -o lsf-output.%J psubmit-mpiexec-wrapper.sh -t lsf -i $jobid_short -n "$n" -p "$PPN" -h "$NTH" -g "$NGPUS" -d "$PSUBMIT_DIRNAME" -e "$TARGET_BIN" -o "$OPTSCRIPT" -a "\"$ARGS\"" >& "$outfile"
