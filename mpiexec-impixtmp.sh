...
for i in $nds; do node=$(echo $i | cut -d: -f1); scp "$TARGET_BIN" ${node}:$newexecname; ssh ${node} "chmod +x $newexecname"; done
...
for i in $nds; do node=$(echo $i | cut -d: -f1); ssh $node "rm -f $newexecname"; done
set +x
time2=$(date +"%s")
