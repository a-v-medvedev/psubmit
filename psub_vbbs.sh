...
function psub_check_job_status() {
    ...
    if [ "$jobstatus" == "Q" ]; then
        ...
        if [ "$NBGJOBS" -eq "0" ]; then 
            ...
            timeout -k20 ${TIME_LIMIT}m $PSUBMIT_DIRNAME/psubmit-mpiexec-wrapper.sh -t vbbs -i $jobid_short -n "$NP" -p "$PPN" -d "$PSUBMIT_DIRNAME" -e "$TARGET_BIN" -o "$OPTSCRIPT" -a "\"$ARGS\"" > "$FILE_OUT" 2>&1 &
