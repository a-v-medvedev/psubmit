...
psub_submit() {
    ...
    sbatch -J "$JOB_NAME" --exclusive --time=${TIME_LIMIT} $resources $blackist $whitelist $account $comment $queue_flag -D "$PWD" -N "$NNODES" -n "$n" $PSUBMIT_DIRNAME/psubmit-mpiexec-wrapper.sh -t slurm -n "$n" -p "$PPN" -h "$NTH" -d "$PSUBMIT_DIRNAME" $xopt  -e "$TARGET_BIN" -o "$OPTSCRIPT" -a "\"$ARGS\"" 2>&1 | tee "$outfile"
